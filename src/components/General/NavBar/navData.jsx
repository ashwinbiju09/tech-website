const navLinks = [
  {
    name: "Products",
    subMenu: [
      {
        name: "Accelerated Products",
        subMenu: [
          { name: "EXIM", path: "#" },
          { name: "E-Invoice", path: "#" },
          { name: "E-Way Bill", path: "#" },
          { name: "GST", path: "#" },
        ],
      },
      {
        name: "Raapyd Products",
        subMenu: [
          { name: "Vendor Management", path: "#" },
          { name: "Field Service Management", path: "#" },
          { name: "Real Estate Management", path: "#" },
          { name: "Asset Management", path: "#" },
          { name: "Distribution Management", path: "#" },
          { name: "Digital Retail Solution", path: "#" },
          { name: "Subscription Billing", path: "#" },
          { name: "Sales Force Automation", path: "#" },
          { name: "Dealer Management System", path: "#" },
        ],
      },
      {
        name: "Eerly AI Products",
        subMenu: [{ name: "Eerly AI Brain", path: "#" }],
      },
    ],
  },
  {
    name: "Solutions",
    subMenu: [
      {
        name: "SAP Consulting",
        subMenu: [
          { name: "SAP S/4HANA", path: "#" },
          { name: "SAP S/4HANA Public Cloud", path: "#" },
          { name: "SAP S/4HANA Private Cloud", path: "#" },
          { name: "SAP Financial Management", path: "#" },
          { name: "SAP Analytics Cloud Planning", path: "#" },
          { name: "SAP Group Reporting", path: "#" },
          { name: "SAP PaPM", path: "#" },
          { name: "SAP Commerce Cloud", path: "#" },
          { name: "SAP Sales Cloud", path: "#" },
          { name: "SAP Service Cloud", path: "#" },
          { name: "SAP Emarsys", path: "#" },
          { name: "SAP E-Mobility", path: "#" },
          { name: "SAP CPI", path: "#" },
          { name: "SAP Build Apps", path: "#" },
          { name: "SAP SuccessFactors HCM", path: "#" },
          { name: "SAP Business AI", path: "#" },
          { name: "SAP Sustainability", path: "#" },
        ],
      },
    ],
  },
  {
    name: "Services",
    subMenu: [
      {
        name: "Strategy and Consulting",
        subMenu: [
          { name: "Business Consulting", path: "#" },
          { name: "Process Consulting", path: "#" },
          { name: "Technology Consulting", path: "#" },
        ],
      },
      {
        name: "SAP Support",
        subMenu: [
          { name: "SAP Managed Services", path: "#" },
          { name: "SAP AMS", path: "#" },
          { name: "SAP S/4HANA Migration", path: "#" },
          { name: "SAP Discovery & Evaluation", path: "#" },
          { name: "SAP Implementation & Rollout", path: "#" },
          { name: "SAP Upgrade Services", path: "#" },
        ],
      },
    ],
  },
  {
    name: "Industries",
    subMenu: [
      {
        name: "Retail",
        subMenu: [
          { name: "Fashion & Apparel", path: "#" },
          { name: "Grocery", path: "#" },
          { name: "Specialty Retail", path: "#" },
          { name: "Healthcare & Wellness", path: "#" },
          { name: "Department Stores", path: "#" },
          { name: "Wholesale Distribution", path: "#" },
          { name: "Gems & Jewelry", path: "#" },
        ],
      },
      {
        name: "Consumer Products",
        subMenu: [
          { name: "Food & Beverage", path: "#" },
          { name: "Consumer Durables", path: "#" },
          { name: "Home & Personal Care", path: "#" },
        ],
      },
      {
        name: "Manufacturing",
        subMenu: [
          { name: "Automotive Manufacturing", path: "#" },
          { name: "Process Manufacturing", path: "#" },
          { name: "Discrete Manufacturing", path: "#" },
        ],
      },
      {
        name: "Engineering & Construction",
        subMenu: [
          { name: "EC&O", path: "#" },
          { name: "Real Estate", path: "#" },
        ],
      },
      {
        name: "Oil, Gas, & Energy",
        subMenu: [
          { name: "Power Generation", path: "#" },
          { name: "Renewable Energy", path: "#" },
          { name: "Energy Retail", path: "#" },
          { name: "Smart Grid", path: "#" },
          { name: "Transmission & Distribution", path: "#" },
        ],
      },
      {
        name: "Professional Services",
        subMenu: [
          { name: "Information Technology", path: "#" },
          { name: "Travel & Tourism", path: "#" },
        ],
      },
      {
        name: "Life Sciences & Healthcare",
        subMenu: [
          { name: "Pharmaceutical & Biotech", path: "#" },
          { name: "Medical Devices", path: "#" },
          { name: "Hospital & Clinics", path: "#" },
        ],
      },
      {
        name: "Banking & Financial Services",
        subMenu: [
          { name: "Banking", path: "#" },
          { name: "Insurance", path: "#" },
          { name: "Financial Institutions", path: "#" },
        ],
      },
      {
        name: "Education",
        subMenu: [
          { name: "Business Information Services", path: "#" },
          { name: "Enterprise Services", path: "#" },
        ],
      },
      {
        name: "High-Tech",
        subMenu: [
          { name: "Computer Peripherals", path: "#" },
          { name: "Contract Manufacturers", path: "#" },
          { name: "Distribution Companies", path: "#" },
        ],
      },
    ],
  },
  { name: "About us", path: "/about" },
  { name: "Contact Us", path: "#" },
];

export default navLinks;
